...
const KEY_PATTERN = /^datasets\/[A-Za-z0-9._-]+\/[A-Za-z0-9._\-:T]+\/[A-Za-z0-9._-]+$/;
...
Deno.serve(async (req) => {
  try {
    ...
    if (!KEY_PATTERN.test(body.r2_key)) {
      return json({ error: "r2_key must look like datasets/<line>/<stamp>/<file>" }, 400);
    }
    ...
    console.error("download-dataset failed:", message);
