# hosted-training-trigger Specification

## Purpose

Document the parked hosted-training integration so the demo can keep manual
Colab as the active path while preserving the requirements for a future paid
GPU provider activation.
...
admin-authenticated request, dispatches a job on a hosted GPU provider, and
links the resulting job id to a new `runs` row when hosted training is
configured.
...
- **THEN** the function inserts a `run_metrics` row with the run id, step,
  epoch, name, and value

#### Scenario: Worker streams a log batch
- **WHEN** the worker posts a `log` event with a valid signature
- **THEN** the function appends the lines to `runs.config_yaml.logs`, trimmed
  to the last 500 lines

#### Scenario: Worker reports success
- **WHEN** the worker posts a `succeeded` event with R2 keys and final metrics
- **THEN** the function inserts a `versions` row linked to the run, updates the
  run's `status` to `succeeded` and `finished_at`, and returns 200

#### Scenario: Worker reports failure
- **WHEN** the worker posts a `failed` event
- **THEN** the function updates the run's `status` to `failed`, appends the
  error to logs, and returns 200
...
The hosted training path SHALL keep all provider API keys and HMAC secrets on
the Supabase Functions runtime; the browser SHALL only ever talk to the Edge
Functions.

#### Scenario: Browser bundle is inspected
- **WHEN** the dashboard's bundled JS is searched for known secret name prefixes
  (`MODAL_`, `REPLICATE_`, `TRAINING_PROVIDER_`, `TRAINING_CALLBACK_`)
...
`scripts/train_yolo26n_seg.py` writes to the registry with a service-role key
SHALL continue to work without changes.

#### Scenario: Local SDK runs in parallel
- **WHEN** an operator runs the Python SDK locally against a model line
- **THEN** the SDK can still insert a `runs` row, stream `run_metrics`, and
  produce a `versions` row with the service-role key
